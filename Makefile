...
$(sd)/surf/%.$(sval).$(resamp_target): $(rtd)
	mri_surf2surf \
		--srcsubject $(SUBJECT) \
...
# 001.mgz: wait for FS to import initial T1
...
$(sd)/dwi/response.txt: $(sd)/dwi/preproc.mif
	dwi2response -force tournier $< $@ -nthreads $(nthread)
...
$(sd)/seeg/CT_in_T1.nii.gz: $(CT) $(fs_done) $(sd)/mri/T1.RAS.nii.gz $(sd)/mri/brain.RAS.nii.gz
...
$(sd)/seeg/seeg.xyz: $(sd)/seeg/labeled_CT.nii.gz $(sd)/seeg/schema.txt
